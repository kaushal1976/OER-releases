...
class oerViewOer extends Jview
{
	function display($tpl = null)
	{
		...
		if($this->getLayout() == 'form') {
			...
			$data	= new stdClass();
			$data	=& $this->get('data');
			$licence_name = $model->getDataLicence($data->licence)->name;
			$licence_description = $model->getDataLicence($data->licence)->decs;
			$licence_url = $model->getDataLicence($data->licence)->url;
			$data ->licence_desc = '<h3>'.$licence_name.'</h3><p>'.$licence_description. '</p><p>'.'<a href="'.$licence_url.'">'.$licence_url.'</a></p>';
			$lists['file'] = '<a href="index.php?option=com_oer&controller=oer&task=download&fname='.$data->filedata.'">'.$data->filedata.'</a>';
