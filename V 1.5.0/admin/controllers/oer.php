...
function listpage()
    ...
    $model = $this->getModel('oer');
    if(!$model->delete()) {
        $msg = JText::_( 'Error: One or records could not be Deleted' );
    } else {
        $msg = JText::_( 'Record(s) Deleted' );
    }
	$this->setRedirect(JRoute::_('index.php?option=com_oer', $msg));
	}
    ...
    $model = $this->getModel('oer');
    if(!$model->publish()) {
        $msg = JText::_( 'Error: One or records could not be published / unpublished' );
    } else {
        $msg = JText::_( 'Record(s) published / unpublished' );
    }
	...
	function save()
	{
		$component = JComponentHelper::getComponent('com_oer');
   		$params = new JParameter( $component->params );
    	$emailto = $params->def('admin_email',0);
